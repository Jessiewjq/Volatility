...
data_original = pd.read_csv('USDT_BTC 30-Minute.csv')
data_original['date'] = data_original['date'].apply(lambda x:
                                               datetime.datetime.fromtimestamp(x).strftime('%Y-%m-%d'))

data_original = data_original.tail(21000)
...
volumep = data_original.loc[:, 'quoteVolume'].tolist()
